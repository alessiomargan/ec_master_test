...
public:
    Ft6ESC(const ec_slavet& slave_descriptor) :
        ...
        Log(std::string("/home/embedded/PDO_log/Ft6ESC_pos"+std::to_string(position)+"_log.txt"),DEFAULT_LOG_SIZE)
    { }
    ...
    virtual int init(const YAML::Node & root_cfg) {
        ...
        log_filename = std::string("/home/embedded/PDO_log/Ft6ESC_"+std::to_string(sdo.sensor_robot_id)+"_log.txt");
